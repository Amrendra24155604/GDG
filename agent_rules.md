...
| **Employee Context Agent** | Fetches employee profile, role, department & active assets from DB. | Verifies employee identity, role eligibility, department, and current device assignment history. | Always **100%** upon valid database user record match. |
...
| Agent Name | One-Line Summary of Work | Operational Rules Followed | Confidence Score Calculation Rules |
| :--- | :--- | :--- | :--- |
| **Employee Context Agent** | Fetches profile and current leave balance counts from DB. | Retrieves user record and extracts remaining `casualLeave`, `sickLeave`, and `earnedLeave` balances. | Always **100%** upon user profile lookup. |
...
| **Recommendation Agent** | Synthesizes leave findings into final manager decision brief. | Combines balance, policy, coverage, and milestone checks to recommend **Approve**, **Reject**, or **Need Review**. | **Formula**:<br>• Start at **100%**<br>• Deduct **40%** if leave balance is insufficient<br>• Deduct **30%** if policy validation fails<br>• Deduct **15%** if team operational risk is HIGH<br>• Deduct **10%** if milestone release conflict exists |
